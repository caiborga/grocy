<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <header v-if="showNav" class="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-gray-100">
            <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
                <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                    <img class="mt-1 h-6 w-46 object-cover" src="/public/grocy.png" />

                    <span class="text-gray-400 select-none">/</span>

                    <span class="font-semibold text-xl text-sky-600 truncate max-w-[200px]"
        ...
        <!-- Bottom Nav (Mobile) -->
        <nav v-if="showNav" class="md:hidden fixed bottom-4 left-4 right-4 z-50">
            <div
                ...
                <RouterLink v-for="item in navItemsMobile" :key="item.to" :to="item.to"
                    class="flex flex-col items-center justify-center gap-1 px-4 py-2 rounded-xl transition-all" :class="isActive(item.to)
